	...
	public int doEndTag() {
		...
		// uri.append("<");
			...
			uri.append((new StringBuilder()).append("<a href=\"JavaScript:").append(function).append("('").append(((nowGroup - 1) * 10) + 1).toString())
					...
					.append("<img src=\"/img/admin/btn/paging_first.gif\" alt=\"처음 목록으로\" /></a>");
		int doStart = nowGroup * pageLimit;
		doStart = ((currentPage % pageLimit) == 0) ? currentPage - (pageLimit - 1) : ((currentPage / pageLimit) * pageLimit) + 1; // doStart
																																	// 로직
																																	// 변경
		...
		if (nowGroup < totalGroup - 1)
			uri.append((new StringBuilder()).append("<a href=\"JavaScript:").append(function).append("('").append(((nowGroup + 1) * 10) + 1).toString())
					.append((new StringBuilder()).append("','").append(nowGroup + 1).append("')\">").toString())
					.append("<img src=\"/img/admin/btn/paging_last.gif\" alt=\"마지막 목록으로\" /></a>");
		else
